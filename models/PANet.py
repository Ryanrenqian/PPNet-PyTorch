...
class OneNet(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        for epi in range(batch_size):
            ...
            if self.training:
                ...
                support_loss_epi = self.supportLoss(supp_fts[:,:,epi],fore_mask[:,:,epi],back_mask[:,:,epi],prototypes)
                sup_loss += support_loss_epi


        output = torch.stack(outputs, dim=1)  # N x B x (1 + Wa) x H x W
        output = output.view(-1, *output.shape[2:])
        output_semantic = torch.zeros(1).to(torch.device('cuda'))
        return output, sup_loss / batch_size, align_loss / batch_size
    ...
    def supportLoss(self, supp_fts, fore_mask, back_mask,prototypes):
        '''
        supp_fts:
        prototypes
        '''
        ...
        for way in range(n_ways):
            # Get the query prototypes
            prototypes = [prototypes[[0]], prototypes[[way + 1]]]
            for shot in range(n_shots):
                img_fts = supp_fts[way, [shot]]
                supp_dist = [self.calDist(img_fts, prototype) for prototype in prototypes]
                supp_pred = torch.stack(supp_dist, dim=1)
